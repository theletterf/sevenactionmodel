...
Ajoutez `--global` pour la rendre disponible dans tous les projets. La compétence s’active pour la planification, la rédaction, les audits, l’architecture de l’information et les métriques de succès de la documentation. Vous pouvez aussi l’invoquer directement :

```text
Utilisez $seven-action-documentation pour auditer notre documentation d’intégration.
```

La compétence aide l’agent à identifier l’action principale du lecteur, à choisir des types de contenu utiles, puis à définir un résultat et une mesure sans inventer de faits sur le produit.
